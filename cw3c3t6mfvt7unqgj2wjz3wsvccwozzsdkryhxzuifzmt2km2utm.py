...
@triton.jit
def _helion_kernel_helion_v8_attention(t_key_cache, t_seq_lens, t_query_start_lens, t_query, t_block_tables, t_value_cache, t_output, t_key_cache_size_3, t_block_tables_stride_1, t_key_cache_stride_0, t_key_cache_stride_1, t_key_cache_stride_2, t_key_cache_stride_3, t_output_stride_0, t_output_stride_1, t_output_stride_2, t_query_stride_0, t_query_stride_1, t_query_stride_2, t_query_start_lens_stride_0, t_value_cache_stride_0, t_value_cache_stride_1, t_value_cache_stride_2, t_value_cache_stride_3, max_query_len, num_seqs, scale, _NUM_SM: tl.constexpr, _RDIM_SIZE_4: tl.constexpr, _RDIM_SIZE_5: tl.constexpr, _SHAPE_DIM: tl.constexpr, _SHAPE_DIM_1: tl.constexpr, _SHAPE_DIM_2: tl.constexpr, _SHAPE_DIM_3: tl.constexpr, mul_1: tl.constexpr, _SHAPE_DIM_10: tl.constexpr, _SHAPE_DIM_11: tl.constexpr, _SHAPE_DIM_18: tl.constexpr, _SHAPE_DIM_19: tl.constexpr, _SHAPE_DIM_20: tl.constexpr, _SHAPE_DIM_21: tl.constexpr, _SHAPE_DIM_22: tl.constexpr):
    ...
    total_pids = tl.cast(num_seqs * tl.cdiv(32, _BLOCK_SIZE_3) * tl.cdiv(max_query_len, _BLOCK_SIZE_0), tl.int64)
    for virtual_pid in tl.range(tl.program_id(0).to(tl.int64), total_pids, _NUM_SM * 2, num_stages=1, disallow_acc_multi_buffer=False, flatten=True):
        # src[helion_unified_attention.py:163]: for seq_tile, tile_m, tile_q in hl.tile(
        # src[helion_unified_attention.py:164]:     [num_seqs, num_query_heads, max_query_len],
        # src[helion_unified_attention.py:165]:     block_size=[1, num_queries_per_kv, q_block_size],
        # src[helion_unified_attention.py:163-166]: ...
        num_blocks_0 = num_seqs
        num_blocks_1 = tl.cdiv(32, _BLOCK_SIZE_3)
        ...
        seq_len = tl.load(t_seq_lens + tl.zeros([], tl.int64), None)
        ...
        add = 1 + offset_2
        query_end = tl.load(t_query_start_lens + add * t_query_start_lens_stride_0, None, eviction_policy='evict_last')
        ...
        for offset_1 in tl.range(0, tl.cast(v_19, tl.int64), _BLOCK_SIZE_1, num_stages=2, disallow_acc_multi_buffer=True, flatten=True):
            ...
            blk_idxs = tl.load(t_block_tables + v_20 * t_block_tables_stride_1, mask_1, other=0, eviction_policy='evict_last')
            ...
            # src[helion_unified_attention.py:208]: k_load = t_key_cache[blk_idxs, :, kv_head_idx, :]
            k_load = tl.load(t_key_cache + (v_21[:, None, None] * t_key_cache_stride_0 + indices_4[None, :, None] * t_key_cache_stride_1 + symnode_0 * t_key_cache_stride_2 + indices_5[None, None, :] * t_key_cache_stride_3), mask_1[:, None, None] & mask_5[None, None, :], other=0, eviction_policy='evict_last')
...
def kernel_helion_v8_attention(t_output, t_query, t_key_cache, t_value_cache, t_block_tables, t_seq_lens, scale, t_query_start_lens, max_query_len, num_seqs, q_block_padded_size: hl.constexpr, batch_size_padded: hl.constexpr, *, _launcher=_default_launcher):
    ...
    # src[helion_unified_attention.py:165]:     block_size=[1, num_queries_per_kv, q_block_size],
    # src[helion_unified_attention.py:163-166]: ...
    _NUM_SM = helion.runtime.get_num_sm(t_output.device)
    _BLOCK_SIZE_0 = 4
    _RDIM_SIZE_4 = 16
    ...
    _launcher(_helion_kernel_helion_v8_attention, (_NUM_SM * 2,), t_key_cache, t_seq_lens, t_query_start_lens, t_query, t_block_tables, t_value_cache, t_output, t_key_cache.size(3), t_block_tables.stride(1), t_key_cache.stride(0), t_key_cache.stride(1), t_key_cache.stride(2), t_key_cache.stride(3), t_output.stride(0), t_output.stride(1), t_output.stride(2), t_query.stride(0), t_query.stride(1), t_query.stride(2), t_query_start_lens.stride(0), t_value_cache.stride(0), t_value_cache.stride(1), t_value_cache.stride(2), t_value_cache.stride(3), max_query_len, num_seqs, scale, _NUM_SM, _RDIM_SIZE_4, _RDIM_SIZE_5, _SHAPE_DIM, _SHAPE_DIM_1, _SHAPE_DIM_2, _SHAPE_DIM_3, 16 * _BLOCK_SIZE_1, _SHAPE_DIM_10, _SHAPE_DIM_11, _SHAPE_DIM_18, _SHAPE_DIM_19, _SHAPE_DIM_20, _SHAPE_DIM_21, _SHAPE_DIM_22, num_warps=4, num_stages=7)
...
def call():
    ...
    t_output = rand_strided(size=(32, 32, 128), stride=(4096, 128, 1), dtype=torch.bfloat16, device='cuda:0')
    t_query = rand_strided(size=(32, 32, 128), stride=(4096, 128, 1), dtype=torch.bfloat16, device='cuda:0')
    ...
    t_block_tables = rand_strided(size=(1, 2), stride=(2, 1), dtype=torch.int32, device='cuda:0')
    t_seq_lens = rand_strided(size=(1,), stride=(1,), dtype=torch.int32, device='cuda:0')
    scale = 1.1
    t_query_start_lens = rand_strided(size=(2,), stride=(1,), dtype=torch.int32, device='cuda:0')
    max_query_len = 32
    num_seqs = 1
    q_block_padded_size = 32
    batch_size_padded = 1
