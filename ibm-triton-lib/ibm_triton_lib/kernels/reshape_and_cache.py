...
def fallback_heuristic(key):
    ret = Config(
        {"THREAD_BLOCK_SIZE": 1024 if key[1] <= 128 else 4096},
        num_warps=16,
        num_stages=4,
    )
    return ret
...
def reshape_and_cache_kernel(
    ...
    tgt_key = tl.load(
        key_ptr + src_key_idx, mask=src_key_idx < (num_tokens * key_stride)
    )
    tgt_value = tl.load(
        value_ptr + src_value_idx, mask=src_value_idx < (num_tokens * value_stride)
    )
    tl.store(
        key_cache_ptr + tgt_key_idx,
        tgt_key,
        mask=tgt_key_idx
        < (
            block_idx * block_size * num_heads * head_size
            + block_size * head_size * num_heads
        ),
    )
    tl.store(
        value_cache_ptr + tgt_value_idx,
        tgt_value,
        mask=tgt_value_idx
        < (
            block_idx * block_size * num_heads * head_size
