...
def kernel_unified_attention_2d(
    ...
    for q_block_global_idx in range(
        tl.program_id(0) * q_block_iterations,
        min((tl.program_id(0) + 1) * q_block_iterations, max_q_block_idx + 1),
    ):
        kv_head_idx = tl.program_id(1)

        if is_prefill:
            seq_idx = find_seq_idx(
                block_q_seq_boundaries_ptr, q_block_global_idx, num_seqs
            )
...
def kernel_unified_attention_3d(
    ...
    for seq_idx in range(
        tl.program_id(0) * seq_idx_iterations,
        min((tl.program_id(0) + 1) * seq_idx_iterations, num_seqs),
    ):
        kv_head_idx = tl.program_id(1)
    ...
    if num_decodes > 0:
        ...
        if not use_split_kv:
            ...
            kernel_unified_attention_3d[
                (LAUNCH_GRID_DIM0_3D_DECODE, num_kv_heads, NUM_SEGMENTS)  # num_decodes,
            ](
